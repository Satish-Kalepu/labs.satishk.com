...
.vas_t { border: 1px solid #cdcdcd; border-radius:2px; cursor:pointer;}
.vas_t .kv { display:block; padding:2px 5px; }
.vas_t .kk { width:30px; text-align:center; float:right; padding:2px 5px; border-left:1px solid #f0f0f0; background-color:#f8f8f8; font-weight:bold; }
.vas_t .kkk { width:30px; text-align:center; float:right; padding:2px 5px; border-left:1px solid #f0f0f0; background-color:#ffaa77; font-weight:bold; color:black; }
.vas_tt { padding:2px 5px; width:100%; padding-left:20px; border: 1px solid #cdcdcd; border-radius:2px; }
.vas_{ padding:2px 5px; border-bottom:1px solid #cdcdcd; cursor:pointer; }
.vas_ span{ color:red;}
.vas_f{ background-color:#f0f0f0; border-bottom:1px solid orange; }
.vas_p {background-color:white; border:1px solid #cdcdcd; width:inherit;}
.vas_td{ max-width:200px; overflow:auto; white-space:nowrap;}
.vas_td{ max-width:200px; max-height:50px; resize:both; overflow:auto; }
.vas_td::-webkit-scrollbar { width: 5px; height: 5px;}
.vas_td::-webkit-scrollbar-track { background: #fff1f1;}
.vas_td::-webkit-scrollbar-thumb { background: #fdd; }
.vas_td::-webkit-scrollbar-thumb:hover { background: #555;}
</style>`);
Vue.component('simpleselect', {
	data: function(){
		return {
			"vselected":{
			},
	...
	methods: {
		initialized: function(){
			...
			if( this.vselected.hasOwnProperty( this.voptions["data"]["search_field"] ) ){
				this.vselected_html = this.vselected[ this.voptions["data"]["search_field"] ];
			}
		},
		copyit: function(v){
		...
		unselect: function(){
			...
			this.vkeyword = "";
			this.hideit();
			this.vselected_html = this.vplaceholder;
			this.inform_parent();
		},
		select_su: function(vi){
			this.vselected = this.su[ vi ];
	...
	<div style=" width:100%; z-index:1;" >
		<div class="vas_t" v-on:click="showit" ><span class='kk' v-html="varrow" ></span><span v-if="vselected[voptions['data']['search_field']]" class='kkk' v-on:click="unselect" >&#9747;</span><span class="kv" v-html="vselected_html"></span></div>
		<div class="vas_p" style="position:absolute;  background-color:white; z-index:5000; min-height:100px;" v-show="vshow" >
			...
			<input ref="suu" class="vas_tt" type="text" v-model="vkeyword" v-on:keyup="vtyped($event)" >
			<div style="background-color:white; max-height:250px; overflow:auto;" >
				<div v-if="voptions['data']['multi_column']==false"   v-bind:ref="'su'+i" v-bind:class="{'vas_':true,'vas_f':v['_f']}" v-for="v,i in su" v-html="v['_v']"  v-on:mouseenter="v['_f']=true" v-on:mouseleave="v['_f']=false" v-on:click="select_su(i)" ></div>
				<table v-if="voptions['data']['multi_column']" class='table table-sm' >
				<tr v-bind:ref="'su'+i" v-bind:class="{'vas_':true,'vas_f':v['_f']}" v-for="v,i in su"  v-on:mouseenter="v['_f']=true" v-on:mouseleave="v['_f']=false" v-on:click="select_su(i)" >
				<td><div class="vas_td" v-html="v['_v']"></div></td><td v-for="z,n in voptions['data']['columns']" ><div class="vas_td" v-html="v[n]"></div></td>
				</tr>
				</table>
			</div>
		</div>
